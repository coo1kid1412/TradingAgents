...
## 8. V1 验收记录（2026-08-02）

### 冻结历史评估

数据集覆盖 2000 年至 2026-07-31，并要求每项特征提供不晚于 `as_of_time` 的可用时间证明。2020-2026 口径明确标记为 `previously_observed_holdout=true`，不是全新盲测。
...
录制行情覆盖租约、数据质量、特征、规则、SQLite、报告与幂等通知，M3 慢路径不计入快路径：
...
离线通知正文已验证首屏包含橙/红灯、立即操作、入场门、仓位上限、持仓动作、数据截至时间、可靠度和“规则分数不是概率”。

### 真实数据探针与安装状态

2026-08-02（周日）使用项目真实 `.env` 探针得到：

- `rt_k_permission=permission_denied`，返回 0 行。
- `realtime_breadth_coverage_pct=0`，横截面不满足生产门槛。
- 非交易日无法完成当天指数实时价格和 `stk_limit` 的交易时段验收。
- 最近已完成交易日识别为 `2026-07-31`。
- `rule_v1/notify` readiness 仅剩 `data smoke is not ready`，因此**没有安装生产 cron，也没有激活 notify/gate**。

下一交易日先在 `09:35` 重跑数据探针。若 `rt_k` 仍为 `permission_denied`，需开通对应实时权限，或接入另一个能够逐行提供扫描时点现价和数据时间、并通过 80% 覆盖率与 5 分钟新鲜度校验的数据源；不得用 T-1 收盘价替代。
