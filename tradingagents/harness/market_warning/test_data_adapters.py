...
class RealtimeAShareAdapterTests(unittest.TestCase):
    ...
    def test_cross_section_emits_coverage_staleness_and_current_limit_down_share(self):
        now = datetime(2026, 7, 20, 10, 0, tzinfo=SHANGHAI)
        stocks = pd.DataFrame(
            [
                {"ts_code": "600000.SH", "last": 9.0, "pre_close": 10.0, "down_limit": 9.0, "data_time": now - timedelta(minutes=1), "source": "tushare_rt_k"},
        ...
        self.assertEqual(values["realtime_breadth_coverage_pct"], 50.0)
