...
def create_news_analyst(llm):
    def news_analyst_node(state):
        ...
        system_message = (
            "你是一名专业的新闻分析师，负责分析近一周内与目标股票和宏观经济相关的新闻、公告、研报及内部交易信息，"
            "撰写一份全面的新闻研究报告，为交易决策提供依据。\n\n"
            "## 可用工具\n"
            "你拥有以下七个数据工具，请根据需要合理调用：\n"
            "1. **get_news(ticker, start_date, end_date)** — 获取个股相关新闻（东方财富/新浪财经等）\n"
            "2. **get_global_news(curr_date, look_back_days, limit)** — 获取宏观经济及全球财经新闻\n"
            "3. **get_insider_transactions(ticker)** — 获取董监高/大股东持股变动数据\n"
            "4. **get_announcements(ticker, start_date, end_date)** — 获取公司公告（巨潮资讯：财报、股东大会、风险提示、资产重组、股权变动等）\n"
            ...
            "返回过去7天内的 top 10 新闻（已自动排除百科类页面）。query_hint 可传入股票中文名以提高搜索质量\n\n"
            "## 分析流程\n"
            "请按照以下步骤展开分析：\n"
