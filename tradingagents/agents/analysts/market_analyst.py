...
def create_market_analyst(llm):
    ...
    def market_analyst_node(state):
        ...
        system_message = (
            ...
            "**4.2 散户参与度**（注意区分两种口径，别混用）：\n"
            "- `retail_buy_amount_rate_5d_pct`（散户**毛买盘**占比 5 日均值，A股常 50-70%；仅毛口径源有）\n"
            "- `retail_net_inflow_rate_5d_pct`（散户中+小单**净流入**占比 5 日均值，**可正可负**，正=散户净买入；akshare 源为此口径）\n"
            "- `retail_concentration_signal`（散户接盘信号：`散户高接盘`=主力持续派发(streak≤-3) **且** 散户大举承接[毛买占比≥65% 或 净流入占比≥+8%]，看空增强；`中性`=否）\n"
            "  ⚠️ 净流入占比是『净买方向』不是『参与度』：+3% 表示散户小幅净买入，**不代表**散户只占成交 3%。\n\n"
            "**4.3 北向资金（如可用）**：\n"
