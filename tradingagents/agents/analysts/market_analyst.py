...
def create_market_analyst(llm):
    ...
    def market_analyst_node(state):
        ...
        system_message = (
            ...
            "**4.2 散户接盘信号**：\n"
            "- ⚠️ **口径排雷**：`retail_buy_amount_rate_5d_pct`（毛买盘占比）当前**数据口径不可靠**（实测稳定 6-10%，远低于应有的 50-70%），`retail_net_inflow_rate_5d_pct`（净流入占比）**几乎恒为 null**——**这两个禁止据其下『散户参与度极低/异常』等解读**（是坏数据不是真信号），最多原样列一句『数据口径存疑』。\n"
            "- `retail_concentration_signal`（散户接盘信号：`散户高接盘`=主力持续派发 **且** 散户大举承接，看空增强；`中性`=否）——可用。\n"
            "- **散户 vs 机构的真信号看 4.4『股东户数环比』**：户数↑=机构派发给散户、户数↓=筹码向机构集中。\n\n"
            "**4.3 北向资金（如可用）**：\n"
