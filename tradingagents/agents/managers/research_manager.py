...
- `SYS_CYCLICAL_CLASS: strong`（强周期：钢/煤/化工/航运/面板/存储等）：
  - 方法 1 = **正常化 PE×EPS（权重 50%）**：EPS **必须**直读 `SYS_CYCLICAL_NORMALIZED_EPS`（= ROE 十年中位 × 当前 BPS，Python 算好，禁自算）；目标 PE 用 **10-15 倍 mid-cycle 带**（对标：成熟强周期行业的穿越周期定价惯例；存储/面板等有结构性改善逻辑的，可论证上浮至 15-20，必须写明产业依据）
  - 方法 2 = **PB×BPS（权重 30%）**：周期股 PB 远比 PE 稳定，目标 PB 引用历史区间/同业
  - 方法 3 = 同业可比 TTM（权重 ≤20%，仅交叉参考）
  - ⛔ **禁 PEG**（周期增速无前瞻意义——顶部增速最高恰是该卖的时候）；⛔ 禁 TTM PE×EPS 当主腿
