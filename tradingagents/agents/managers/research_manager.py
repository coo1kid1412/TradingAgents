...
- `SYS_CYCLICAL_CLASS: strong`（强周期：钢/煤/化工/航运/面板/存储等）：
  - 方法 1 = **正常化 PE×EPS（权重 50%）**：EPS **必须**直读 `SYS_CYCLICAL_NORMALIZED_EPS`（= ROE 十年中位 × 当前 BPS，Python 算好，禁自算）；**目标 PE 必须用 10-15 倍 mid-cycle 带**（对标：成熟强周期行业的穿越周期定价惯例；存储/面板等有结构性改善逻辑的，可论证上浮至 15-20，必须写明产业依据）
  - ⛔ `SYS_CYCLICAL_PE_ON_NORMALIZED` 是"当前价÷正常化EPS"的**贵不贵读数**，**严禁当目标倍数用**——拿它算目标价 = 目标价恒等于现价的套套逻辑（000725 实跑犯过，下游审计会标记）
  - 方法 2 = **PB×BPS（权重 30%，必须有）**：周期股 PB 远比 PE 稳定，目标 PB 引用历史区间/同业
  - 方法 3 = 同业可比 TTM（权重 ≤20%，仅交叉参考）
  - ⛔ **禁 PEG，无例外**（周期增速无前瞻意义——顶部增速最高恰是该卖的时候）。强周期股画像**不会有** SYS_PEG/SYS_TARGET_PRIMARY 行（源头已禁发）；若 VALUATION_METHOD 正文写了 peg，一律无视
...
- `SYS_CYCLICAL_NORMALIZED_EPS: N/A`（年度样本不足）→ 退回 PB×BPS 主导（50%）+ 同业（30%）+ 历史分位（20%）
- ⛔ **无例外条款**：周期路由与 VALUATION_METHOD.primary_method / SYS_TARGET_PRIMARY 冲突时，**一律周期路由优先**。**禁止**"混合路由""实际采用""综合考虑成长叙事"式的自行折中——你没有在两条机读指令之间做裁量的权限；strong 周期股 PEG 全禁、无例外（000725 实跑曾以"成长叙事"为由把 PEG 拉回当 40% 主腿，属违规，下游审计会标记）
