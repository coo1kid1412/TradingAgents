...
- `SYS_CYCLICAL_CLASS: strong`（强周期：钢/煤/化工/航运/面板/存储等）——**目标价已由 Python 算死，直读 `SYS_CYCLICAL_TARGET` 行，禁现场重算**：
  ...
  - **直读画像 `SYS_CYCLICAL_TARGET: bear=__~__ | bull=__~__ | base=__~__ | 离散=__x | 置信=__`**：
    ...
    * **`base` 就是你的综合目标价区间，原样采用**（已按周期位置滑动权重做概率加权，权重 Python 锁死）。
    * ⛔ **禁止自己用 SYS_CYCLICAL_NORMALIZED_EPS / SYS_FORWARD_EPS 重算两腿、禁止自选 PE 倍数、禁止重做加权平均**——历史 bug：兆易同股同输入，RM 一次正常化腿 50%(目标230→SELL)、一次 0%(目标723→HOLD)、又一次 SELL↔UW，全是现场重算的漂移。现在 Python 算死、你直读，摆动根除。
  - **`置信=low`（离散≥2.5x，双峰）时**：base 是折中锚、非真公允值；在第五步情景里把 bear/bull 作为悲观/乐观两端显式列出，且评级临近阈值时**偏保守不下强方向单**。
