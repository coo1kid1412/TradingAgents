...
| PEG | 高增长公司 | **必须调 `compute_peg_target_price`**：① `eps`/`growth_rate_pct` 入参直读 `SYS_FORWARD_EPS`/`SYS_PEG_GROWTH_PCT`（确定性，已半衰封顶+低基数护栏，**严禁**自选）；② **`target_peg_low`/`target_peg_high` 入参必须直读画像 `SYS_PEG_BAND: low=__ high=__`**（regime 派生的确定性 PEG 倍数带，**严禁自拍 0.8↔2.4 乱选**——澜起式同股 PEG 腿 267-401↔167-234 摆动，就是 RM 一次用 PEG~2.0、一次用 ~1.2 乱跳造成的）；③ ⛔ **PEG 腿目标价必须原样采用工具返回的 `low`/`mid`/`high`，严禁手算、严禁自行设隐含 PE、严禁"觉得公式目标太低就上调"**（隐含 PE = `PEG区间 × growth_rate_pct`，由工具算）。仅当画像无 SYS_PEG/SYS_PEG_BAND 行时才退回自估 |
