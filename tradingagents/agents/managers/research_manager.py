...
⚠️ **PE/EPS 口径一致铁律**：`目标 PE` 与所乘 `EPS` 必须**同口径**。stock_profile 的 `target_pe_range` 锚自**同业 TTM 中位**（TTM 口径）时，必须乘 **EPS_TTM**，**不得**乘前瞻 EPS（EPS_TTM×(1+增速)）——否则双重计入成长、目标价虚高近 50%（高成长股会被错误抬成强买）。只有当目标 PE 本身是前瞻 PE 时，才配前瞻 EPS。
