...
## ⚠️ 数值类论据的评分注意事项
- **PE 估值类论据**：评定时以分析师报告中「系统计算」的 PE 值为准。如果辩论双方引用了不同的 PE 值（如多头用 19 倍、空头用 42 倍），应指出数据引用矛盾并在评分中体现——数据不一致的论据应扣分
...
- 多头得分 - 空头得分 > 1 → **Buy**
- 空头得分 - 多头得分 > 1 → **Sell**
- 得分差 ≤ 1 → **Hold**（多空势均力敌，不强行站队）

**重要约束**：
- 得分差 ≤ 1 时，说明多空论据说服力接近，LLM 评分的随机波动就可能翻转方向，此时不应做出方向性决策，应选择 Hold 维持现状观察。
- 得分差 > 1 时，方向信号明确，必须选择对应方向（Buy 或 Sell），不要回避决策。
- Hold 不代表"无观点"，而是"当前证据不足以支撑方向性操作"，需在投资计划中说明后续观察的触发条件。
