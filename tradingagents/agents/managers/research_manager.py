...
- `SYS_CYCLICAL_CLASS: strong`（强周期：钢/煤/化工/航运/面板/存储等）——**两腿位置驱动滑动权重**：
  存储/面板这类「结构性上行周期」既有周期风险（顶部峰值盈利不可线性外推）、又有 AI 结构性需求（正常化会低估），故**正常化腿 与 成长前瞻腿 按周期位置滑动配比**，权重由画像 `SYS_CYCLICAL_TARGET_WEIGHTS` 行**锁死，禁自选**：
  - **正常化腿**：EPS **必须**直读 `SYS_CYCLICAL_NORMALIZED_EPS`（= ROE 十年中位 × 当前 BPS，禁自算）× **10-15 倍 mid-cycle 带**（顶部按下沿、谷底按上沿）。⛔ `SYS_CYCLICAL_PE_ON_NORMALIZED` 是"贵不贵读数"，**严禁当目标倍数用**（拿它算 = 目标价恒等于现价的套套逻辑）。
  - **成长前瞻腿**：EPS 直读 `SYS_FORWARD_EPS`（强周期股现已注入，确定性）× PEG 带（调 `compute_peg_target_price`），反映 AI 结构性需求。
  - **配比**：直读画像 `SYS_CYCLICAL_TARGET_WEIGHTS: normalized=__ growth=__`（按 position 锁定：顶部 0.7/0.3 偏谨慎、中段 0.5/0.5、谷底 0.3/0.7 偏乐观）。综合目标价 = 正常化腿 × normalized 权重 + 成长腿 × growth 权重，**两个权重照抄此行,禁自选/禁因"成长叙事"上调成长权重**。同业可比 TTM 仅作交叉校验，不进权重。
  - ⛔ **禁用 primary_method 改判**：strong 周期股**忽略** stock_profile.VALUATION_METHOD.primary_method（哪怕它=peg）和 SYS_TARGET_PRIMARY；**权重唯一来源是 SYS_CYCLICAL_TARGET_WEIGHTS**。历史 bug：兆易同股同输入，RM 一次给正常化腿 50%(目标230→SELL)、一次 0%(目标723→HOLD)，3 倍跳。
