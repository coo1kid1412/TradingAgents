...
⛔ **确定性字段直读（优先于自行判断）**：fundamentals 报告里若有 `【SYS_LANDMINE｜tushare】` 行（Python 确定性输出），其中 `ST=` / `连续两年亏损=` / `资产负债率=` / `流动比率=` 四个字段**必须照抄该行的值**判定 L1/L2/L6 对应项，禁止从 prose 重新猜；该行标注"不可得"的项（质押率/商誉占比/审计意见等）才自行从 news/fundamentals 正文判断，判断不出写"无信号"。
...
| PEG | 高增长公司 | ⛔ **PEG 腿目标价直读画像 `SYS_PEG_TARGET_PRICE: low=__ mid=__ high=__`（Python 已算死），严禁自己调 `compute_peg_target_price`、严禁手算、严禁自设隐含 PE、严禁"觉得公式目标太低就上调"**。此三值 = `SYS_FORWARD_EPS×(SYS_PEG_BAND×SYS_PEG_GROWTH_PCT)`，确定性，根治天孚式同股同输入 PEG 腿 194↔269↔342-456 乱跳（根因：RM 调工具时无视 SYS 值、自塞高一倍的增速/PEG，把 SELL 抬成 HOLD）。**仅当画像无 `SYS_PEG_TARGET_PRICE` 行时**，才退回调 `compute_peg_target_price`（入参 `eps`/`growth_rate_pct` 直读 `SYS_FORWARD_EPS`/`SYS_PEG_GROWTH_PCT`，`target_peg_low`/`high` 直读 `SYS_PEG_BAND`，原样采用返回值）；二者皆无时才自估 |
...
⛔ **盈利质量 PEG 禁用闸**：画像若出现 `SYS_PEG_ELIGIBLE: false` 或
`SYS_PEG_CONFIDENCE: invalid`，PEG 方法完全禁用：不得调用 PEG 工具、不得自行估算 PEG、
不得把归母高增速换个名称塞回目标价。改用主营正常化估值 + 可验证金融资产净值折价 + 净现金的
分部思路；关键持股数量、税费或流动性折价缺失时，明确写“目标价数据不足”，置信度最高为低，
不能用无效 PEG 抬高目标价后再通过评级同号不变量强制得到 HOLD。
...
##### ⛔ 周期股口径路由（最高优先级；SYS_CYCLICAL_CLASS 存在时覆盖成长路由与默认表）

画像末尾若有 `SYS_CYCLICAL_CLASS:` 行（Python 确定性识别：传统周期行业关键词 + 科技周期名单），**必须按周期口径执行**。依据是林奇铁律：周期股**顶部 TTM PE 最低（该卖）、谷底 TTM PE 最高（该买）**，TTM 口径整个反着——这也是为什么 TTM EPS 在这里禁止当主腿：

- `SYS_CYCLICAL_CLASS: strong`（强周期：钢/煤/化工/航运/面板/存储等）——**目标价已由 Python 算死，直读 `SYS_CYCLICAL_TARGET` 行，禁现场重算**：
  存储/面板这类「结构性上行周期」既有周期风险（顶部峰值盈利不可线性外推）、又有 AI 结构性需求（正常化会低估）。两个估值视角常相差数倍，**严禁算术平均成一个谁都不信的中间数**——Python 已按情景概率加权算好，你只需照抄。
  - **直读画像 `SYS_CYCLICAL_TARGET: bear=__~__ | bull=__~__ | base=__~__ | 离散=__x | 置信=__`**：
    * `bear`＝周期均值回归视角（正常化 EPS × mid-cycle PE），`bull`＝结构成长视角（前瞻 PEG）；
    * **`base` 就是你 Step 4 的最终综合目标价区间，原样采用、到此为止**。base **已经是** bear 与 bull 按周期位置权重的概率加权结果（bull 已含在内）。
    * 🛑🛑 **强周期股 Step 4 只有 SYS_CYCLICAL_TARGET 这一个目标价来源，综合目标价 = base 区间，直接采用**：
      - ⛔ **禁止再把 PEG/前瞻腿（=bull）当成"另一个方法"和 base 一起加权**——bull 已经在 base 里了，再加一遍就是**成长腿双重计数**（兆易实测：base 151.5 被错误顶到 232）。
      - ⛔ **禁止调用 `compute_weighted_target_price` 去混合 base 和其它方法**；强周期股不走多方法加权，base 即终值。
      - ⛔ 禁止自己用 SYS_CYCLICAL_NORMALIZED_EPS / SYS_FORWARD_EPS 重算两腿、禁选 PE 倍数、禁重做加权——历史 bug：兆易同股同输入 230↔723↔SELL↔UW 全是现场重算漂移。Python 算死、你直读 base，摆动根除。
    * **bear / bull 的去向：只进 Step 5 的 Bear case / Bull case 情景**（分别作悲观/乐观端目标价），不进 Step 4 综合目标价的加权。
  - **`置信=low`（离散≥2.5x，双峰）—— Python 确定性判定，你必须照用，禁止用"前瞻vs后视镜口径差不算真分歧"之类说辞合理化掉**：在第五步把 bear/bull 作悲观/乐观两端显式列出，且评级临近阈值时**偏保守不下强方向单**。
  - ⛔ **禁用 primary_method 改判**：strong 周期股**忽略** stock_profile.VALUATION_METHOD.primary_method（哪怕它=peg）和 SYS_TARGET_PRIMARY；目标价唯一来源是 `SYS_CYCLICAL_TARGET`。
  - 同业可比 TTM 仅作交叉校验，不进目标价。`SYS_CYCLICAL_PE_ON_NORMALIZED` 是"贵不贵读数"，严禁当目标倍数用。
- `SYS_CYCLICAL_CLASS: semi`（半周期：封测/代工等有成长β）：正常化 PE×EPS 腿 50% + 原成长路由腿（SYS_TARGET_PRIMARY/PEG）50%；同业仅交叉校验不进权重
- `SYS_CYCLICAL_POSITION: trough`（谷底）：⛔ 禁以"TTM PE 高"为由看空（谷底 PE 虚高是周期常态）；`top`（顶部）：⛔ 禁以"TTM PE 低"为由看多
- `SYS_CYCLICAL_NORMALIZED_EPS: N/A`（年度样本不足，无 SYS_CYCLICAL_TARGET_WEIGHTS 行）→ 退回 PB×BPS 主导（50%）+ 同业（30%）+ 历史分位（20%）

##### ⛔ 成长股前瞻口径路由（次高优先级，覆盖上表默认；周期路由存在时让位）

stock_profile 末尾若有 `SYS_TARGET_PRIMARY_METHOD:` 行（Python 确定性注入），**必须照它执行**，不许用 VALUATION_METHOD 正文里的 primary_method 改判：
- `SYS_TARGET_PRIMARY_METHOD: peg` → **方法 1 必须是 PEG（前瞻 EPS，权重 50%）**；TTM 口径的 `PE×EPS` 腿权重**压到 ≤20%**（它是后视镜，不主导成长龙头 12 个月目标）；剩余权重给方法 3。
- **⛔ 方法 3 锚固定（禁随机挑，治澜起 274↔189 摆动）**：方法 3（权重 30%）**优先用卖方目标价锚**（consensus.MARKET_IMPLIED_VALUATION.sell_side_target_price_range 或 news 卖方目标价中位）；**仅当卖方目标价确实缺失时，才退回同业可比 TTM**。不许"这次挑卖方、下次挑同业"——两者数值差异大（澜起卖方 200-320 vs 同业 157-190），随机挑就是摆动。用了哪个必须写明 + 注明另一个为何不用。
- 典型权重 = PEG 50% / 方法3(卖方目标价优先) 30% / PE×EPS(TTM) 20%。
...
- **PEG 前瞻增速分段衰减**：年度增速 ≤40% 全采信；超出 40% 的部分打五折；封顶 60%。前瞻 EPS = EPS_TTM × (1 + 前瞻增速)——既不让异常高增被纯线性外推，也不把可持续高增压得过狠。**画像已用此公式算好 `SYS_FORWARD_EPS` / `SYS_PEG_GROWTH_PCT`，PEG 腿直读即可，不要自己再算一遍（自算会引入 LLM 漂移）**
...
若为 `invalid`，执行 Step 4 的盈利质量 PEG 禁用闸，不适用上述“低置信但仍可使用 PEG”的规则。
