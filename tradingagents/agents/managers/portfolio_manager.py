...
**规则（避免重复劳动）**：Bull / Base / Bear 三档**直接复用 RM Step 5 的输出**（目标价、概率、核心假设原文照抄），你只需新增 **黑天鹅 Tail** 一档。这避免你和 RM 对同一情景给出不一致的目标价或概率。
...
**约束**：
- Bull/Base/Bear 三档**禁止修改 RM 给的目标价或概率**——若你认为 RM 错，应在第二步反向质疑里指出，而非这里悄悄改
- 黑天鹅档**必须**显式引用尾部风险分析师的论据
- 加入黑天鹅档后，**Bull/Base/Bear 三档的原始概率之和需要从 100% 等比例收缩到 (100% - tail%)**——例如 RM 给 25/50/25，你加 10% Tail，则三档变为 22.5/45/22.5
