...
**"热门概念归属"填写约束**：
- **占主营营收% 必须直读基本面报告的 `SYS_MAIN_BUSINESS:` 行**（Python 从 fina_mainbz 确定性算的产品营收占比，如"化合物半导体材料 13% / 红外系锗 11%"），**禁止自己推断占比**。把相关产品线归并到概念时，% 为对应产品线占比之和，并在括号注明是哪些产品线（如"磷化铟/化合物半导体 ~13%（化合物半导体材料）"）。
...
1. **Trade Ticket 决策卡**（上述格式：At-a-glance + 核心交易参数 + 关键背景 + **热门概念归属** + **各 Agent 核心结论一览**）
