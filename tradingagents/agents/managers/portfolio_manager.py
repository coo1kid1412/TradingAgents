...
| **`holder_num_qoq_pct`（股东户数环比%）** | **筹码迁徙——散户 vs 机构最有体感的信号**：环比**↑**=户数增加=机构把筹码派给散户=**派发/顶部信号**；环比**↓**=户数减少=筹码向机构集中=**吸筹**。缺失则写"股东户数数据缺失"，**不要拿别的指标硬凑** |
| `retail_concentration_signal` | 散户接盘信号：`散户高接盘`=主力派发 **且** 散户大举承接（看空增强）；`中性`=否 |

⚠️ **散户口径排雷（强制）**：`retail_buy_amount_rate_5d_pct`（毛买盘占比）**当前数据口径不可靠**（实测稳定 6-10%，远低于 A 股应有的 50-70%）、`retail_net_inflow_rate_5d_pct`（净流入占比）**几乎恒为 null**——**这两个一律不要写进快照、更禁止据其编"散户参与度极低/异常"之类的解读**（是坏数据不是真信号）。散户侧只用 **股东户数环比** + **散户接盘信号** 表达。
...
| 当前赔率 | R = U/D = X.XX |
| 概率加权期望收益 | E = X.XX% |
| **资金面快照（主力 vs 散户）** | **主力**：近5日净流入 __亿/近20日 __亿（capital_flow_score __/100，regime __，DDX 1年分位 __）；**筹码迁徙**：股东户数环比 __%（↑=机构派发给散户/顶部，↓=筹码集中/吸筹；缺失则写"股东户数数据缺失"）；**散户接盘**：__（散户高接盘/中性）；近 __ 日主力连续净流入/流出 → 主导方为 **主力/散户** |
